...
<script type="text/javascript" language="javascript">
	function saveData() {
		...
		var url = "${contextPath}/servlet/CategoryServlet?task=updateCategory&date="
				+ Math.random() + "";
		jQuery.post(url, formData, function(jsonData) {
			if (jsonData.flag == false) {
				window.alert(jsonData.message);
			} else {
				var confirmFlag = window.alert("类别修改成功");
			}
		}, "json");
	}
</script>
...
<body>
	<div id="dcWrap">
		...
		<div id="dcMain">
			<!-- 当前位置 -->
			<div id="urHere">
				商城后台管理中心<b>></b><strong>修改分类</strong>
			</div>
			<div class="mainBox"
				style="height: auto !important; height: 550px; min-height: 550px;">
				<h3>
					<a href="${contextPath}/servlet/CategoryServlet?task=list" class="actionBtn">商品分类</a>修改分类
				</h3>
				<form id="categoryForm" action="#" method="post">
					<table width="100%" border="0" cellpadding="8" cellspacing="0"
						...
						<tr>
							...
							<td><input type="text" name="cid" id="cid" value="${cid}"
								readonly="readonly" size="40" class="inpMain" /></td>
						</tr>
						<tr>
							<td align="right">上级分类</td>
							<td><select name="parentid" id="parentid" style="width: 292px">
									<option value="0">作为一级分类</option>
									<c:forEach var="pcategory" items="${parentList}">
										<option value="${pcategory.cid}" ${pid==pcategory.cid ? "selected='selected'":""}>${pcategory.classname}</option>
									</c:forEach>
						...
						<tr>
							...
							<td><input type="text" name="classname" id="classname" value="${editcategoryBean.classname}" size="40"
								class="inpMain" /></td>
						</tr>
